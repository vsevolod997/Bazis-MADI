...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func sceneDidBecomeActive(_ scene: UIScene) {
        ...
        //relogin user
        if let userData = userLoginData.getUserData() {
            HttpService.getUserAccount(login: userData.login, password: userData.password) { (err, userMod, userErrModel) in
                if userErrModel != nil {
                    if let windowScene = scene as? UIWindowScene {
                        self.showLogin(windowScene: windowScene)
                    }
                }
            }
        }
